...
## Day 7 - Network fully redundant, so measuring lead-time impact instead of reachability loss

Decision: the disruption simulation found 0 single points of failure — every customer region stays reachable no matter which warehouse or DC is removed, because Day 3's shipment generation samples origin/destination freely across all warehouses and DCs rather than restricting each node to a fixed set of trade partners. Rather than regenerating the data with artificial lane restrictions, kept the network as-is and added a second metric: average delivery lead-time increase per customer region if a node fails, plus a customer redundancy score (number of distinct DCs serving each customer). Why: "0 disruptions" is a legitimate finding, not a bug, but it made the dashboard page uninteresting on its own. The lead-time slowdown metric gives a continuous, differentiated result (Germany's DC0004 stands out clearly: +6.3 days average, +12.9 days worst-case, across 20 customer regions) without needing to fabricate artificial fragility into the network. Trade-off / risk: the network's actual topology (every DC can reach every customer) is less realistic than a real supply chain, where a given customer is usually served by 1-3 DCs, not all of them. This is documented here rather than hidden, in case it comes up in an interview.